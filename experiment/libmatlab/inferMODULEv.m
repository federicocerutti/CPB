function node = inferMODULE(node)
...
while ~isempty(active),
    ...
    if schild==0,
        if isempty(node(id).back),
            if isempty(node(id).value),
                if isempty(node(id).childmsg),
                    ...
                    node(id).back = feval(fusionfunction,node(id).childmsg);
                end
            else
        ...
        node(idx).childmsg(loc,:) = feval(backfunction,node(id).wv,node(id).parentmsg,sendid,node(id).back);
        ...
        if isempty(node(id).forward),
            if isempty(node(id).value),
                if isempty(node(id).parentmsg),
                    node(id).forward = node(id).wv(:)';
                else
                    node(id).forward = feval(forwardfunction,node(id).wv,node(id).parentmsg);
                end
            else
        ...
        node(idx).parentmsg(loc,:) = feval(fusionfunction,[node(id).forward; node(id).childmsg([1:sendid-1 sendid+1:end],:)]);
...
for id=1:N,
    ...
    node(id).we = feval(fusionfunction,[node(id).forward; node(id).back]);
end