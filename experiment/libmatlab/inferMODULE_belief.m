...
while ~isempty(active),
    ...
    if schild==0,
        ...
        node(idx).childmsg(loc,:) = feval(backfunction,node(id).w,node(id).parentmsg,sendid,node(id).back);
        ...
        if isempty(node(id).forward),
            if isempty(node(id).value),
                if isempty(node(id).parentmsg),
                    ...
                    node(id).forward = feval(forwardfunction,node(id).w,node(id).parentmsg);
                end
            else
