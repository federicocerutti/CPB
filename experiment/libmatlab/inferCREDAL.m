function node = inferCREDAL(node)
...
while ~isempty(active),
    ...
    if schild==0,
        if isempty(node(id).back),
            if isempty(node(id).value),
                if isempty(node(id).childmsg),
                    ...
                    node(id).back = fusion_credal(node(id).childmsg);
                end
            else
        ...
        node(idx).childmsg(loc,:) = backprop_credal(node(id).w,node(id).parentmsg,sendid,node(id).back);
        ...
        if isempty(node(id).forward),
            if isempty(node(id).value),
                if isempty(node(id).parentmsg),
                    ...
                    node(id).forward = forwardprop_credal(node(id).w,node(id).parentmsg);
                end
            else
        ...
        node(idx).parentmsg(loc,:) = fusion_credal([node(id).forward; node(id).childmsg([1:sendid-1 sendid+1:end],:)]);
...
for id=1:N,
    ...
    node(id).we = fusion_credal([node(id).forward; node(id).back]);
end